...
function AuthStore(options, callback){
    ...
    self.get = function (sessionId, callback) {
        self.collection.findOne({_id: new ObjectID(sessionId)}, callback);
    };
    ...
    self.destroy = function(sid, callback){
        self.collection.findAndRemove({_id: sid}, function(err, data){
            if(callback) callback(err, data);
        });
    };
